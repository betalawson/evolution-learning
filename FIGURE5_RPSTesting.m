function FIGURE5_RPSTesting(regenerate)
...
for P = 1:length(problems_list)
    ...
    param_filenames = {'f10','f20','f30','b1','c1','b2','c2','b3','c3'};
    num_suffixes = {'a','b','c','d','e','f','g','h','i'};
    N_figs = length(param_filenames);
    fig_filenames = cell(1,N_figs);
    for k = 1:N_figs
        fig_filenames{k} = ['FIG5',num_suffixes{k},'_RPS_',param_filenames{k}];
    end
...
f10LS_mat = zeros(N_pops, 2, N_rep);
f20LS_mat = zeros(N_pops, 2, N_rep);
f30LS_mat = zeros(N_pops, 2, N_rep);
b1LS_mat = zeros(N_pops, 2, N_rep);
c1LS_mat = zeros(N_pops, 2, N_rep);
b2LS_mat = zeros(N_pops, 2, N_rep);
c2LS_mat = zeros(N_pops, 2, N_rep);
b3LS_mat = zeros(N_pops, 2, N_rep);
c3LS_mat = zeros(N_pops, 2, N_rep);
l2LS_mat = zeros(N_pops, 2, N_rep);
f10EL_mat = zeros(N_pops, 2, N_rep);
f20EL_mat = zeros(N_pops, 2, N_rep);
f30EL_mat = zeros(N_pops, 2, N_rep);
b1EL_mat = zeros(N_pops, 2, N_rep);
c1EL_mat = zeros(N_pops, 2, N_rep);
b2EL_mat = zeros(N_pops, 2, N_rep);
c2EL_mat = zeros(N_pops, 2, N_rep);
b3EL_mat = zeros(N_pops, 2, N_rep);
c3EL_mat = zeros(N_pops, 2, N_rep);
...
for k = 1:N_pops
    ...
    for type = test_types
        ...
        parfor r = 1:N_rep
            ...
            [f10,f20,f30,b1,c1,b2,c2,b3,c3] = extractRPSParams(A,type);

            % Extract parameters
            f10LS_mat(k,type,r) = f10;
            f20LS_mat(k,type,r) = f20;
            f30LS_mat(k,type,r) = f30;
            b1LS_mat(k,type,r) = b1;
            c1LS_mat(k,type,r) = c1;
            b2LS_mat(k,type,r) = b2;
            c2LS_mat(k,type,r) = c2;
            b3LS_mat(k,type,r) = b3;
            c3LS_mat(k,type,r) = c3;
            ...
            % Extract parameters
            f10EL_mat(k,type,r) = f10;
            f20EL_mat(k,type,r) = f20;
            f30EL_mat(k,type,r) = f30;
            b1EL_mat(k,type,r) = b1;
            c1EL_mat(k,type,r) = c1;
            b2EL_mat(k,type,r) = b2;
            c2EL_mat(k,type,r) = c2;
            b3EL_mat(k,type,r) = b3;
            c3EL_mat(k,type,r) = c3;
...
f10_mat(:,1:2,:) = f10LS_mat;
f10_mat(:,3:4,:) = f10EL_mat;
f20_mat(:,1:2,:) = f20LS_mat;
f20_mat(:,3:4,:) = f20EL_mat;
f30_mat(:,1:2,:) = f30LS_mat;
f30_mat(:,3:4,:) = f30EL_mat;
b1_mat(:,1:2,:) = b1LS_mat;
b1_mat(:,3:4,:) = b1EL_mat;
c1_mat(:,1:2,:) = c1LS_mat;
c1_mat(:,3:4,:) = c1EL_mat;
...
f10_perfect = zeros(1,2);
f20_perfect = zeros(1,2);
f30_perfect = zeros(1,2);
b1_perfect = zeros(1,2);
c1_perfect = zeros(1,2);
b2_perfect = zeros(1,2);
c2_perfect = zeros(1,2);
b3_perfect = zeros(1,2);
c3_perfect = zeros(1,2);
...
for type = test_types
    ...
    % Extract parameters
    f10_perfect(type) = f10;
    f20_perfect(type) = f20;
    f30_perfect(type) = f30;
    b1_perfect(type) = b1;
    c1_perfect(type) = c1;
    b2_perfect(type) = b2;
    c2_perfect(type) = c2;
    ...
    % Extract parameters
    f10_perfect(type+2) = f10;
    f20_perfect(type+2) = f20;
    f30_perfect(type+2) = f30;
    b1_perfect(type+2) = b1;
    c1_perfect(type+2) = c1;
    b2_perfect(type+2) = b2;
    c2_perfect(type+2) = c2;
    b3_perfect(type+2) = b3;
    c3_perfect(type+2) = c3;
...
[f10_true,f20_true,f30_true,b1_true,c1_true,b2_true,c2_true,b3_true,c3_true] = extractRPSParams(A,type);

% Store all data in a single results object and save it
simResults = struct('f10_mat',f10_mat,'f20_mat',f20_mat,'f30_mat',f30_mat,'b1_mat',b1_mat','c1_mat',c1_mat,'b2_mat',b2_mat','c2_mat',c2_mat,'b3_mat',b3_mat','c3_mat',c3_mat, ...
                    'f10_perfect',f10_perfect,'f20_perfect',f20_perfect,'f30_perfect',f30_perfect,'b1_perfect',b1_perfect','c1_perfect',c1_perfect,'b2_perfect',b2_perfect','c2_perfect',c2_perfect,'b3_perfect',b3_perfect','c3_perfect',c3_perfect, ...
                    'f10_true',f10_true,'f20_true',f20_true,'f30_true',f30_true,'b1_true',b1_true','c1_true',c1_true,'b2_true',b2_true','c2_true',c2_true,'b3_true',b3_true','c3_true',c3_true, ...
                    'l2_mat',l2_mat,'l2_perfect',l2_perfect,'pop_sizes',pop_sizes,'LS_time_tot',LS_time_tot,'EL_time_tot',EL_time_tot);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function [f10,f20,f30,b1,c1,b2,c2,b3,c3] = extractRPSParams(A,type)
%
