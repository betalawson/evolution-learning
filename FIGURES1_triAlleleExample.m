...
load('problems_inheritance.mat','allele_transient');
problem = allele_transient;
problem.obs_Nth = 1;
%problem.N_gen = 100;
problem.N_pop = 5000;
...
[library, fitfun] = evolutionLearning(trajWFs_nonD, problem.selection_type, ELoptions);
...
ELoptions.regression_type = 'ls';
library = evolutionLearning(trajWFs_nonD, problem.selection_type, ELoptions);
...
for n = 1:length(problem.X0)
    ...
    evec = zeros( length(problem.X0),1 );
    evec(n) = 1;
    ...
    plot(traj.rep.t, evec' * fitfun{1}.F(traj.rep.t / problem.t_gen), 'k--', 'LineWidth', 2, 'color', [0.5 0.5 0.5]);
