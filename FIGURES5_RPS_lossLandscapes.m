function FIGURES5_RPS_lossLandscapes(regenerate)
...
p1_true = 0.3;            % True fitness advantage for 1st vs 2nd
p2_true = 0.3;            % True fitness penalty for 1st vs 3rd
X0 = [0.2;0.5;0.3];       % Initial proportions
...
% Set data filename 
data_filename = 'DATA_RPS_lossSurfaces.mat';

% Define the type II replicator RHS for the two allele case
fitness_fun = @(x,p1,p2) feval(fitnessRPS(3,2,[p1,0.8,0.2],[p2,0.8,0.2]),x);
...
if regenerate || ~exist(data_filename, 'file')
    ...
    for i = 1:Ny
        ...
        for j = 1:Nx
            ...
            [T,x_here] = runReplicator(3, t_gen, tpts, @(x) fitness_fun(x,p1m(i,j),p2m(i,j)), X0);
            x_here = x_here';
...
shading flat;
view(2);
xlabel('Win strength for 1st feature, $b_1$','Interpreter','LaTeX');
ylabel('Loss strength for 1st feature, $c_1$','Interpreter','LaTeX');
set(gca,'FontSize',24);
